  ...
  try {
    if (!hasSmtpCredentials()) {
      return res.status(503).json({
        message: "Email service is not configured. Please set SMTP credentials in backend/.env"
      });
    }
    ...
    await sendContactEmail({ topic, name, email, message });
    res.json({ success: true, message: "Your message has been sent successfully." });
  } catch (err) {
    console.error("[contact] submission failed", err.message || err);
    res.status(500).json({
...
async function start() {
  ...
  try {
    ...
    console.warn("[server] MongoDB unavailable — vehicle/booking APIs may not work. Contact form still available.");
  }
}
